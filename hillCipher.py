# A python program to illustrate Hill Cipher Technique
import numpy as np
...
def make_key():
    ...
    C = None
    while True:
        cipher = input("Input 4 letter cipherKey: ")
...
def create_matrix_of_integers_from_string(string):
    ...
    integersList = [chr_to_int(c) for c in string]
    length = len(integersList)
    # intializing the matrix
    M = np.zeros((2, int(length / 2)), dtype=np.int32)
    iterator = 0
    for column in range(int(length / 2)):
        for row in range(2):
            M[row][column] = integersList[iterator]
            iterator += 1
    return M
...
if __name__ == "__main__":
    m = input("Message: ")
    cipherText = encryption(m)
    print("Cipher-Text:-->>", cipherText)  # printing cipher text
    decryptedText = decryption(cipherText)
    print("Decrypted-Text:-->>", decryptedText) # printing decrypted text
